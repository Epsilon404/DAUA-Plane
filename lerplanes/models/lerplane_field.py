...
from lerplanes.models.attention.ExternalAttention import ExternalAttention
from lerplanes.models.attention.SEAttention import SEAttention
from lerplanes.models.attention.SKAttention import SKAttention
from lerplanes.models.attention.TripletAttention import TripletAttention
from lerplanes.models.attention.ECAAttention import ECAAttention
...
def interpolate_ms_features(pts: torch.Tensor,
                            ms_grids: Collection[Iterable[nn.Module]], # ms_grids=self.grids
                            grid_dimensions: int, # grid_dimensions=self.grid_config[0]["grid_dimensions"]
                            concat_features: bool, # concat_features=self.concat_features
                            num_levels: Optional[int], # num_levels=None
                            att,
                            mode: str = 'bilinear',
                            ) -> torch.Tensor:
...
class LerplaneField(nn.Module):
    def __init__(
        ...
        if self.linear_decoder:
            ...
            self.sigma_net = tcnn.Network(
                n_input_dims=self.feature_dim, # 64
            ...
            self.in_dim_color = ( # 31
                direction_encoder_n_output_dims # 0
                + self.geo_feat_dim # 15
                + self.appearance_embedding_dim # 0
                + self.pt_encoded_dim # 16
            )
    ...
    def get_density(self, pts: torch.Tensor, timestamps: Optional[torch.Tensor] = None):
        ...
        pts = pts.reshape(-1, pts.shape[-1]) # [262144,4]
        features = interpolate_ms_features(  # [262144,64]
    ...
    def forward(self,
        ...
        if not self.linear_decoder:
            ...
            if self.encode_info.get('encode_items', None) is not None:
                ...
                if self.encode_info.get('encode_items') == 'sigma_out':
                    ...
                    # print(encoded_items.shape) # [xxx,4] (pts and timestamps)
                else:
                    encoded_items = pts.reshape(-1, pts.shape[-1])[:, [
                        xyz[i] for i in self.encode_info.get('encode_items') if i != 't']]
                encoded_pt = self.pt_encoder(encoded_items) # 4 -> 16
        ...
        if self.use_appearance_embedding: # False
