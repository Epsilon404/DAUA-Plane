...
def _load_endo_uncer_map_image(idx: int,
                            paths: List[str],
                            data_dir: str,
                            out_h: int,
                            out_w: int,
     ...
     f_path = paths[idx]
     # load pred_depth, all values are integers
     uncer = imageio.imread(f_path, format="PNG-PIL", ignoregamma=True).astype(np.float32)
     if uncer.shape[0] != out_h or uncer.shape[1] != out_w:
          # use lanczos to resize
          uncer = uncer.resize((out_w, out_h), Image.LANCZOS)
     # use torch.from_numpy to convert to tensor
     uncer = torch.from_numpy(uncer).float().unsqueeze(-1)
...
def _parallel_loader_endo_uncer_map_image(args):
    torch.set_num_threads(maxT)
    return _load_endo_uncer_map_image(**args)
...
def parallel_load_uncer_map(data_type:str, **kwargs):
    if data_type == 'endo':
        return parallel_load_endo_uncer_map(**kwargs)
    # elif data_type == 'hamlyn':
    #     return parallel_load_hamlyn_uncer_map(**kwargs)
    else:
        raise NotImplementedError

def parallel_load_endo_uncer_map(tqdm_title: str, num_images: int, **kwargs) -> Tuple[List[Any], List[Any]]:
    max_threads = maxT
    fn = _parallel_loader_endo_uncer_map_image
    outputs = parallel_load_images_wrappers(
        max_threads, num_images, fn, tqdm_title, **kwargs)
    return outputs
