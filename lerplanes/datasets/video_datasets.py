...
from .data_loading import parallel_load_images, parallel_load_depth, parallel_load_endo_mask, parallel_load_uncer_map
...
class VideoEndoDataset(BaseDataset):
    ...
    def __init__(self,
        ...
        imgs = parallel_load_images(
            data_dir=datadir,
            dset_type='llff',
            tqdm_title=f"Loading {split} data, imgs",
        ...
        depths = parallel_load_depth(
            ...
            data_dir=datadir,
            tqdm_title=f"Loading {split} data, depths",
        ...
        uncer_map = parallel_load_uncer_map(
            data_type = 'hamlyn' if 'hamlyn' in datadir else 'endo',
            data_dir=datadir,
            tqdm_title=f"Loading {split} data, uncer",
            num_images=len(paths_uncer),  # Need to be modifieds
            paths=paths_uncer,
            out_h=intrinsics.height,
            out_w=intrinsics.width,
        )
        ...
        if 'hamlyn' in datadir:
            ...
            uncer_map = [i[:500, :, :] for i in uncer_map]
        ...
        imgs, self.masks, self.depths, self.uncer = [torch.cat(lst, dim=0) for lst in [imgs, masks, depths, uncer_map]]
        ...
        if split == 'train':
            ...
            self.uncer = self.uncer.view(-1, self.uncer.shape[-1])
        ...
        print(split,'self.timestamps.shape',self.timestamps.shape)
    ...
    def __getitem__(self, index):
        ...
        if hasattr(self, 'depths'):
            ...
            out['uncer'] = self.uncer[index].view(-1, self.uncer.shape[-1])
        return out
