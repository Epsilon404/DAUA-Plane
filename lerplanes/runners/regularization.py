...
class DepthLossHuber(Regularizer):
    ...
    def compute_depth_loss_huber(self, gt_depth, pred_depth, gt_uncer, mask=None):
        if mask is not None:
    ...
    def _regularize(self, model: LowrankModel, model_out, **kwargs) -> torch.Tensor:
        ...
        if self.weight > 0:
            ...
            target_uncer = kwargs['data']['uncer']
            # get valid mask
            ...
            if self.what == 'field':
                ...
                total += self.compute_depth_loss_huber(target_depth, depth, target_uncer, valid_mask)
            elif self.what == 'proposal_network':
